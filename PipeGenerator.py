...
class PipeGenerator:
    def __init__(self, pipe_group: Group, asset_factory: AssetFactory, game_configuration: GameConfiguration):
        ...
        self._max_num_pipes = int(self._game_configuration.window_width
    ...
    def __create_pipe__(self, pipe_x: int) -> None:
        random_pos = random.randint(self._min_pipe_height,
                                    self._game_configuration.window_height - self._min_pipe_height - self._game_configuration.pipe_gap_height)
        ...
        bottom_pipe = self.__create_pipe_part__(pipe_x, self._game_configuration.window_height - random_pos,
                                                self._game_configuration.pipe_width, random_pos, PipeDirection.Up)

        self._pipe_group.add(top_pipe)
        self._pipe_group.add(bottom_pipe)

    def __create_pipe_part__(self, x: int, y: int, width: int, height: int, pipe_direction: PipeDirection) -> Pipe:
